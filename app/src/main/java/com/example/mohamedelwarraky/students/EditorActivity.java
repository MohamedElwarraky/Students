...
public class EditorActivity extends AppCompatActivity implements
    ...
    /**
     * EditText field to enter the student's presence
     */
     ...
     * {@link StudentEntry#UNKNOWN}, {@link StudentEntry#ONE_DAY},
     * {@link StudentEntry#TWO_DAYS}, {@link StudentEntry#THREE_DAYS}, or
     * {@link StudentEntry#FOUR_DAYS},.
     */
    private int mPresence = StudentEntry.UNKNOWN;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPresenceSpinner = findViewById(R.id.spinner_gender);
        ...
        mPresenceSpinner.setOnTouchListener(mTouchListener);
        mFirstDegreeEditText.setOnTouchListener(mTouchListener);
        mSecondDegreeEditText.setOnTouchListener(mTouchListener);
        mThirdDegreeEditText.setOnTouchListener(mTouchListener);
        mFourthDegreeEditText.setOnTouchListener(mTouchListener);
    ...
    /**
     * Setup the dropdown spinner that allows the user to select the presence of the student.
     */
    private void setupSpinner() {
        // Create adapter for spinner. The list options are from the String array it will use
        // the spinner will use the default layout
        ArrayAdapter presenceSpinnerAdapter = ArrayAdapter.createFromResource(this,
                R.array.array_presence_options, android.R.layout.simple_spinner_item);

        // Specify dropdown layout style - simple list view with 1 item per line
        presenceSpinnerAdapter.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line);

        // Apply the adapter to the spinner
        mPresenceSpinner.setAdapter(presenceSpinnerAdapter);

        // Set the integer mSelected to the constant values
        mPresenceSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String selection = (String) parent.getItemAtPosition(position);
                if (!TextUtils.isEmpty(selection)) {
                    if (selection.equals(getString(R.string.one_day))) {
                        mPresence = StudentEntry.ONE_DAY;
                    } else if (selection.equals(getString(R.string.two_days))) {
                        mPresence = StudentEntry.TWO_DAYS;
                    } else if (selection.equals(getString(R.string.three_days))) {
                        mPresence = StudentEntry.THREE_DAYS;
                    } else if (selection.equals(getString(R.string.four_days))) {
                        mPresence = StudentEntry.FOUR_DAYS;
                    } else {
                        mPresence = StudentEntry.UNKNOWN;
                    }
                }
            }
    ...
    private void saveStudent() {
        ...
        String[] DegreeString = new String[]{
                mFirstDegreeEditText.getText().toString().trim(),
                mSecondDegreeEditText.getText().toString().trim(),
                mThirdDegreeEditText.getText().toString().trim(),
                mFourthDegreeEditText.getText().toString().trim()
        };
        ...
        if (mCurrentStudentUri == null &&
                ...
                TextUtils.isEmpty(DegreeString[2]) && TextUtils.isEmpty(DegreeString[4])
                && mPresence == StudentEntry.UNKNOWN) {
        ...
        values.put(StudentEntry.COLUMN_STUDENT_PRESENCE, mPresence);
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            int presence = cursor.getInt(presenceColumnIndex);

            // Update the views on the screen with the values from the database
            mNameEditText.setText(name);
            mGroupEditText.setText(group);
            String[] input = convertStringToArray(degree);
            Log.v("EditorActivity", "input.length " + input.length);
            if (input.length > 0) {
                mFirstDegreeEditText.setText(input[0]);
                Log.v("EditorActivity", "input " + input[0]);
            } else
                mFirstDegreeEditText.setText("");
            if (input.length > 1) {
                mSecondDegreeEditText.setText(input[1]);
                Log.v("EditorActivity", "input " + input[1]);
            } else
                mSecondDegreeEditText.setText("");
            if (input.length > 2) {
                mThirdDegreeEditText.setText(input[2]);
                Log.v("EditorActivity", "input " + input[2]);
            } else
                mThirdDegreeEditText.setText("");
            if (input.length > 3) {
                mFourthDegreeEditText.setText(input[3]);
                Log.v("EditorActivity", "input " + input[3]);
            } else
                mFourthDegreeEditText.setText("");
            // Gender is a dropdown spinner, so map the constant value from the database
            // into one of the dropdown options (0 is Unknown, 1 is Male, 2 is Female).
            // Then call setSelection() so that option is displayed on screen as the current selection.
            switch (presence) {
                case StudentEntry.ONE_DAY:
                    mPresenceSpinner.setSelection(1);
                    break;
                case StudentEntry.TWO_DAYS:
                    mPresenceSpinner.setSelection(2);
                    break;
                case StudentEntry.THREE_DAYS:
                    mPresenceSpinner.setSelection(3);
                    break;
                case StudentEntry.FOUR_DAYS:
                    mPresenceSpinner.setSelection(4);
                    break;
                default:
                    mPresenceSpinner.setSelection(0);
                    break;
            }
        }
    }
    ...
    public void onLoaderReset(Loader<Cursor> loader) {
        ...
        mThirdDegreeEditText.setText("");
        mFourthDegreeEditText.setText("");
        mPresenceSpinner.setSelection(0); // Select "Unknown" days
    }
